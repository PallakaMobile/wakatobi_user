package ps.salam.wakatobi.support.fab;
...
import ps.salam.wakatobi.support.fab.listener.OnRapidFloatingActionListener;
import ps.salam.wakatobi.support.fab.listener.OnRapidFloatingButtonSeparateListener;
...
public class RapidFloatingActionButton extends FrameLayout implements View.OnClickListener {
    ...
    private void parserAttrs(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        try {
            ...
            normalColor = a.getColor(R.styleable.RapidFloatingActionButton_rfab_color_normal, ContextCompat.getColor(getContext(), R.color.rfab__color_background_normal));
            pressedColor = a.getColor(R.styleable.RapidFloatingActionButton_rfab_color_pressed, ContextCompat.getColor(getContext(), R.color.rfab__color_background_pressed));
    ...
    private void initAfterConstructor() {
        ...
        // 中间图片大小24dp
        buttonDrawableSize = dip2px(getContext(), RFABConstants.SIZE.RFAB_DRAWABLE_SIZE_DP);
    ...
    private void refreshRFABDisplay() {
        if (null == buttonDrawable) {
            buttonDrawable = getResourceDrawableBounded(getContext(), DEFAULT_BUTTON_DRAWABLE_RES_ID, buttonDrawableSize);
        }

        // 设置rfab的背景图片
        CircleButtonDrawable normalDrawable = new CircleButtonDrawable(getContext(), rfabProperties, normalColor);
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
            this.setBackground(selectorClickSimple(normalDrawable, new CircleButtonDrawable(getContext(), rfabProperties, pressedColor)));
        } else {
            this.setBackgroundDrawable(selectorClickSimple(normalDrawable, new CircleButtonDrawable(getContext(), rfabProperties, pressedColor)));
        }
    ...
    private Drawable getResourceDrawableBounded(Context context, int drawableResId, int bound) {
    ...
    private StateListDrawable selectorClickSimple(Drawable normal, Drawable pressed) {
