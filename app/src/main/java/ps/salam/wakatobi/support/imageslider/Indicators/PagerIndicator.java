package ps.salam.wakatobi.support.imageslider.Indicators;
...
import ps.salam.wakatobi.support.imageslider.SliderAdapter;
...
public class PagerIndicator extends LinearLayout implements ViewPagerEx.OnPageChangeListener {
    ...
    /**
     * bind this Indicator with {@link ps.salam.wakatobi.support.imageslider.Tricks.ViewPagerEx}
     */
...
//    private int mUnSelectedGradientDrawable;
//    private int mSelectedGradientDrawable;
...
//        mDefaultSelectedColor = attributes.getColor(R.styleable.PagerIndicator_selected_color, ContextCompat.getColor(context, R.color.white));
//        mDefaultUnSelectedColor = attributes.getColor(R.styleable.PagerIndicator_unselected_color, ContextCompat.getColor(context, R.color.black_trans50));
...
//                mUnSelectedGradientDrawable.setShape(GradientDrawable.RECTANGLE);
    ...
    public void setIndicatorStyleResource(int selected, int unselected) {
        ...
        if (selected == 0) {
            mSelectedDrawable = R.drawable.indicator_selected;
        } else {
//            mSelectedDrawable = ContextCompat.getDrawable(mContext,mUserSetSelectedIndicatorResId);
        }
        if (unselected == 0) {
            mUnselectedDrawable = R.drawable.indicator_unselected;
        } else {
...
//            mUnSelectedGradientDrawable.setSize((int) w, (int) h);
    ...
    private void resetDrawable() {
        for (View i : mIndicators) {
            if (mPreviousSelectedIndicator != null && mPreviousSelectedIndicator.equals(i)) {
                ((ImageView) i).setBackgroundResource(mSelectedDrawable);
            } else {
                ((ImageView) i).setBackgroundResource(mUnselectedDrawable);
            }
        }
    }
    ...
    public void redraw() {
        ...
        for (int i = 0; i < mItemCount; i++) {
            ...
            indicator.setBackgroundResource(mUnselectedDrawable);
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(getResources().getDimensionPixelSize(R.dimen.dimen10),
                    getResources().getDimensionPixelSize(R.dimen.dimen10));
            lp.setMargins(getResources().getDimensionPixelSize(R.dimen.dimen2), 0, 0, 0);
        ...
        public void onChanged() {
            ...
            if (count > mItemCount) {
                for (int i = 0; i < count - mItemCount; i++) {
                    ...
                    LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(getResources().getDimensionPixelSize(R.dimen.dimen10),
                            getResources().getDimensionPixelSize(R.dimen.dimen10));
                    lp.setMargins(getResources().getDimensionPixelSize(R.dimen.dimen5), 0, 0, 0);
                    indicator.setLayoutParams(lp);
                    indicator.setLayoutParams(lp);
                    indicator.setBackgroundResource(mUnselectedDrawable);
                    indicator.setPadding((int) mUnSelectedPadding_Left,
    ...
    private void setItemAsSelected(int position) {
        if (mPreviousSelectedIndicator != null) {
            ...
            mPreviousSelectedIndicator.setBackgroundResource(mUnselectedDrawable);
        ...
        if (currentSelected != null) {
            currentSelected.setBackgroundResource(mSelectedDrawable);
